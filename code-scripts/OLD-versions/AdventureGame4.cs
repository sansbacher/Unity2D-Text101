...
using TMPro;					// Add the TextMesh Pro namespace to access the various functions/types

// Last Updated Jan 6, 2019
// Unity 2D course, Text 101
// With added code for a randomized correct departure gate, and needing to meet objectives, and a clock/time limit
...
public class AdventureGame : MonoBehaviour {
	...
	[SerializeField] Text textComponent;			// a "textbox" type component in Unity (on the Canvas) -> output  (UI.Text)
	[SerializeField] TextMeshProUGUI clockDisplay;		// a Text Mesh Pro "textbox" in Unity. TMPro.TextMeshProUGUI similar to UI.Text type in Unity
	...
	[SerializeField] State outOfTimeState;          // If the clock runs out then jump to this state
	[SerializeField] int maxGameMinutes;	        // How many minutes the Player has
	...
	int hourOnClock = 4;						// Static, the player is only give X minutes
	int minsOnClock = 0;						// How many mins past the hour have elapsed, increased with each State (generally 1)
	...
	void Update () {
		...
		UpdateStoryTextAndClock();				// Update the display/textComponent
	}

	// update the Canvas textComponent with the State's story text, customizing for certain States. And updates the Clock
	private void UpdateStoryTextAndClock() {
		if (state == startingState) {
			minsOnClock = 0;
			correctDepartureGateNum = -1;				// reset in case playing again
			textComponent.text = state.GetStateStory();
		} else if (state == flightInfoState) {
		...
		clockDisplay.text = GetClockTime();
	}

	// Returns the time in HH:MM PM format for the Clock display
	private string GetClockTime() {
		if (minsOnClock < 0) {
			minsOnClock = 0;
		}
		return String.Format("{0}:{1} PM", hourOnClock.ToString(), minsOnClock.ToString("00"));
	}
	...
	private void ManageState() {
		...
		for (int i = 0; i < nextStates.Length; i++) {
			...
			if (Input.GetKeyDown(KeyCode.Alpha1 + i)) {
				...
				state = VerifyNextState(nextStates[i]);		// Update the current state to one of the available nextStates, check the Time, and Objectives if about to succeed
				...
				minsOnClock = minsOnClock + state.GetStateMinutes();		// Update the time
				//Debug.Log("Current State: " + state.GetStateName());		// Prints only if there is a key pressed, once per key press
			}
		}
	}

	// Verifies a possible Next State the Player is about to move to, to ensure all Objectives have been met (if about to Finish), and that time hasn't run out
	private State VerifyNextState(State potentialNextState) {
		if (minsOnClock > maxGameMinutes) {
			return outOfTimeState;						// Player ran out of time
		}
